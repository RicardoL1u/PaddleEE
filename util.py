...
def generate_query(arg,start,end,trigger)->str:
    if arg == "object":
        ...
        query = "处于位置&%d&和位置-%d-之间提到的*%s*事件的行动对象为?比如会议,活动,项目,计划,任务,以及组织,公司,人名" % (start, end, trigger)
    elif arg == "time":
        query = "处于位置&%d&和位置-%d-之间提到的*%s*事件发生的时间为?比如年,月,日,季度,时刻,节日" % (start, end, trigger)
    elif arg == "location":
        query = "处于位置&%d&和位置-%d-之间提到的*%s*事件发生的地点为?比如国家,城市,乡镇,大洲,公共场所,学校" % (start, end, trigger)
